...
class Driver:
    ...
    def __init__(self):
        ...
        # Verify that the socket provided hostname is usable
        try:
            port = self.portManager.newPort()
            tuples = socket.getaddrinfo(self.host, port, socket.AF_INET, socket.SOCK_STREAM)
        except:
            self.host = "localhost"
...
#        self.requestManager.cacheStatistics()
    ...
    def doWait(self, args):
        ...
        if not ok:
            ...
            # Reported Timeout - try to run server
            reverse_events = {event: key for key, event in checkEvents.items()}
            for pending_event in pending:
                rid = reverse_events[pending_event]  # find rid of pending request
                requestEvent = self.eventManager.findEvent(True, rid)
                responseEvent = self.eventManager.findEvent(False, rid)

                # see if the server ever received the request
                if responseEvent is None or responseEvent.receivedHeaderLines == []:
                    if self.verbose:
                        self.console.outMsg("Timeout BEFORE server received request")

                    # Mark that this event has timed out (False for isRequest)
                    self.timeout_events[rid] = False
                    self.eventManager.changeTag(requestEvent, "timeout on request", "timeout")
    ...
    def trace(self, rid):
        ...
        if responseEvent is None or responseEvent.receivedHeaderLines == []:
            if rid in self.timeout_events:
                self.console.outMsg("Timeout before the server could receive the request")
            else:
